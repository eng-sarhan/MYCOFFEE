...
def signup(request):
    if request.method == 'POST' and 'btnsignup' in request.POST:
        ...
        if fname and lname and address and address2 and city and state and zip and email and username and password:
                         ...
                         if User.objects.filter(email=email).exists():
                             ...
                             if re.match(patt,email):
                                 # Add user
                                  ...
                                  # success message
                                  messages.success(request,'Congratulation!, Your account created succefully')
                                  is_added = True
                             else:
           ...
           messages.error(request,'Chech empty fields')
...
def profile(request):
        if request.method == 'POST' and 'btnprofile' in request.POST:
            # To update username data    لتعديل بيانات المستخدم

            if request.user is not None and request.user.id is not None :
                userprofile = UserProfile.objects.get(user=request.user)

                if (request.POST['fname'] and request.POST['lname'] and
                request.POST['address'] and request.POST['address2'] and
                request.POST['city'] and request.POST['state'] and
                ...
                request.POST['pass']):
                    ...
                    #email & username does not change
